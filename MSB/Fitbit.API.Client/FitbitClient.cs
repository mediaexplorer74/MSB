...
namespace Fitbit.API.Client
{
    public partial class FitbitClient : BaseClient
    {
        ...
        public async Task<AuthTokenResponse> Authenticate(string authCode, string redirectUri)
        {
            ...
            try
            {
                ...
                {
                    string resultJson = await response.Content
                        .ReadAsStringAsync().ConfigureAwait(false);
                    AuthTokenResponse result = JsonConvert
                        .DeserializeObject<AuthTokenResponse>(resultJson);
            ...
            try
            {
                ...
                if (response.Content != null)
                {
                    ...
                    AuthTokenResponse result = 
                        JsonConvert.DeserializeObject<AuthTokenResponse>(resultJson);
        ...
        public void SetBasicAuthorizationHeader()
        {
            ...
            string b64Encode = 
                System.Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(baseHeaderVal));
            Client.DefaultRequestHeaders.Authorization = 
                new System.Net.Http.Headers.AuthenticationHeaderValue("Basic", b64Encode);
        }
        ...
        public string SerializeToQueryString<T>(T item)
        {
            ...
            Dictionary<string, string> dict = JsonConvert.DeserializeObject<Dictionary<string, 
                string>>(json);
    ...
    public class BaseClient
    {
        ...
        public void SetAuthToken(string authToken, string tokenType)
        {
            AuthToken = authToken;
            Client.DefaultRequestHeaders.Authorization =
                new System.Net.Http.Headers.AuthenticationHeaderValue(tokenType, AuthToken);
        }
        ...
        protected async Task<TResult> PostAsync<TResult>(string query)
        {
            try
            {
                ...
                {
                    // throw new Exception(string.Format("No content for PostAsync {0}", query));
                    Debug.WriteLine(string.Format("No content for PostAsync {0}", query));
                }
