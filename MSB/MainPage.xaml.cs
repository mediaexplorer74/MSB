...
namespace FitBand {
    public sealed partial class MainPage : Page {
        ...
        {
            StoredRefreshToken = await StoredSettings.TryLoad(
        ...
        WebViewNavigationCompletedEventArgs args) 
        {
            ...
            if (args.IsSuccess)//(RedirectedToAuthPage(args.Uri.ToString())) 
            {
                ...
                else if (FragmentContainsAccessToken(args.Uri.Fragment)) //implicit oauth returns info in fragment and not query
                {
                    ...
                    Fitbit.API.Model.Activities.GetUserActivityLogsResponse activities = await Client.GetUserActivityLogs(new DateTime(2015, 6, 30));
                    WebViewControl.Navigate(HomeUri);
